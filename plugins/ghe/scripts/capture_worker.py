#!/usr/bin/env python3
"""
Capture Worker for GHE Transcription System

This subprocess handles the actual message capture work.
It's spawned by capture_user.py after the hook returns JSON immediately.

Reads input from environment variables:
- GHE_PROMPT: User's prompt text
- GHE_TRANSCRIPT_PATH: Path to transcript file
- GHE_CWD: Working directory
"""
...
def debug_log(message: str, level: str = "INFO") -> None:
    ...
    try:
        # Use GHE_CWD if available, otherwise current directory
        cwd = os.environ.get("GHE_CWD", str(Path.cwd()))
        log_file = Path(cwd) / ".claude" / "hook_debug.log"
        log_file.parent.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S,%f")[:-3]
        with open(log_file, "a") as f:
            f.write(f"{timestamp} {level:<5} [capture_worker] - {message}\n")
    except Exception:
        pass
...
def spawn_transcription_worker() -> None:
    """Spawn the transcription worker to post messages to GitHub."""
    try:
        ...
        debug_log(f"Failed to spawn transcription worker: {e}", "ERROR")


def main() -> None:
    """Main entry point for capture worker."""
    debug_log("capture_worker started")

    # Read input from environment variables
    prompt = os.environ.get("GHE_PROMPT", "")
    transcript_path = os.environ.get("GHE_TRANSCRIPT_PATH", "")
    ...
    # STEP 3: Spawn transcription worker if we captured messages
    if messages_captured > 0:
        spawn_transcription_worker()
        debug_log(f"Total {messages_captured} messages captured")

    debug_log("capture_worker completed")
