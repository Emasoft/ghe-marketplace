...
def acquire_merge_lock_safe(issue_num: str) -> bool:
    ...
    result = ghe_gh('issue', 'list', '--label', lock_label, '--state', 'open',
                    '--json', 'number,updatedAt', capture=True)
            ...
            if lock_data:
                ...
                if lock_epoch != 0:
                    ...
                    if lock_age > LOCK_TTL:
                        log_warn(f"Stale lock detected on issue #{lock_issue} (age: {lock_age}s > TTL: {LOCK_TTL}s)")
                        log_info("Force-releasing stale lock...")
                        ghe_gh('issue', 'edit', lock_issue, '--remove-label', lock_label)
                    else:
                        log_warn(f"Lock held by issue #{lock_issue} (age: {lock_age}s, TTL: {LOCK_TTL}s)")
...
def reconcile_ghe_state() -> None:
    ...
    if config_issue and config_issue != "null":
        result = ghe_gh('issue', 'view', config_issue, '--json', 'state',
                       '--jq', '.state', capture=True)
        ...
        if issue_state == "CLOSED":
            ...
            try:
                ...
                content = re.sub(r'^current_issue:.*$', 'current_issue: null', content, flags=re.MULTILINE)
                content = re.sub(r'^current_phase:.*$', 'current_phase: null', content, flags=re.MULTILINE)
    ...
    if worktrees_dir.is_dir():
        for wt in worktrees_dir.glob('issue-*/'):
            if wt.is_dir():
                ...
                result = ghe_gh('issue', 'view', issue_num, '--json', 'state',
                              '--jq', '.state', capture=True)
                issue_state = result.stdout.strip() if result.returncode == 0 else "UNKNOWN"

                if issue_state == "CLOSED":
                    log_warn(f"Orphaned worktree found for closed issue #{issue_num}: {wt}")
...
def validate_with_retry(script: str, file: str) -> bool:
    ...
    for i in range(1, MAX_RETRIES + 1):
        ...
        if i < MAX_RETRIES:
            log_warn(f"Validation failed (attempt {i}/{MAX_RETRIES}), retrying in {RETRY_DELAY}s...")
...
def pre_flight_check(issue_num: str) -> bool:
    ...
    result = ghe_gh('issue', 'list', '--label', 'merge:active', '--state', 'open',
                   '--json', 'number', '--jq', '.[0].number', capture=True)
...
def recover_from_merge_crash(issue_num: str) -> None:
    ...
    if worktree_path.is_dir():
        ...
        if rebase_merge.is_dir() or rebase_apply.is_dir():
            ...
            subprocess.run(['git', 'rebase', '--abort'], cwd=str(worktree_path), check=False)

        # Check for merge in progress
        merge_head = worktree_path / ".git" / "MERGE_HEAD"
        if merge_head.is_file():
            log_warn("Found in-progress merge, aborting...")
            subprocess.run(['git', 'merge', '--abort'], cwd=str(worktree_path), check=False)
...
def main() -> int:
    ...
    parser.add_argument('command', nargs='?', help='Command to run (help, preflight, reconcile, recover)')
